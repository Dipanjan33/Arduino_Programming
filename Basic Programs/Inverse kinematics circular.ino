...
void loop() {
  ...
  //takes co ordinate input and does inverse kinematics on it and then gives the output

  for (int x = 100; x <= 120; x++) {
    int y = sqrt((20 + x - 100)(20 - x + 100)) + 65;
    coordinate[0] = 0;
    coordinate[1] = x;
    coordinate[2] = y;
    inverse_kinematics();
    for (int i = 0; i < 3; i++) {
      while (desired_angle[i] != angle[i]) {
        if ((millis() - t) > 10) {
          t = millis();
          angle[i] += (desired_angle[i] > angle[i]) ? 1 : -1;
        }
      }
    }
  }
  for (int x = 120; x >= 100; x--) {
    int y = abs(-sqrt((20 + x - 100)(20 - x + 100)) + 65);
    coordinate[0] = 0;
    coordinate[1] = x;
    coordinate[2] = y;
    inverse_kinematics();
